...
class WorkerThread {
 public:
  ...
  void run_loop(uint64_t num_pages) {
    ...
    auto pages_to_write_per_iter =
        std::min(page_log_ival, total_pages_to_write);
    ...
    while ((pages_read + pages_written) < num_pages) {
      ...
      auto pages_to_read_eff =
          std::min(pages_to_read_per_iter, remaining_pages_to_read);
...
class StatisticsThread {
 public:
    ...
    while (not terminate) {
      ...
      if (logging_enabled) {
        log_file << setprecision(2) << setfill('0') << get_iso8601_time()
                 << " read_mibps:" << fixed << read_rate
                 << " write_mibps:" << fixed << write_rate << "\n";
        log_file.flush();
      }
  ...
  string get_iso8601_time() {
    ...
    const auto now_ms{chrono::duration_cast<std::chrono::milliseconds>(
